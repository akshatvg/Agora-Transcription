...
$("#stop-note").click(function () {
  ...
  recognition.onresult = function (event) {
    ...
    document.getElementById("note-text").innerHTML = "<b><i>UserID(" + options.uid + "):</i></b> " + Content;
  };
  $("#note").attr("disabled", false);
  $("#stop-note").attr("disabled", true);
  $("#stop-transcribe").attr("disabled", true);
...
$("#stop-transcribe").click(function () {
  ...
  recognition.onresult = function (event) {
    ...
    var transcript = event.results[current][0].transcript;
    Content = Content + transcript + "<br>";
    document.getElementById("actual-text").innerHTML = "<b><i>UserID(" + options.uid + "):</i></b> " + Content;
    singleMessage = Content;
    channel.sendMessage({ text: singleMessage }).then(() => {
      console.log("Message sent successfully.");
      console.log("Your message was: " + singleMessage + " by " + accountName);
      $("#actual-text").append("<br> <b>Sender:</b> " + accountName + "<br> <b>Message:</b> " + singleMessage + "<br>");
    }).catch(error => {
    ...
    channel.on('ChannelMessage', ({ text }, senderId) => {
      ...
      $("#actual-text").append("<br> <b>Sender:</b> " + senderId + "<br> <b>Message:</b> " + text + "<br>");
    });
  };
  $("#note").attr("disabled", false);
  $("#stop-note").attr("disabled", true);
